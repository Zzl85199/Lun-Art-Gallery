...
- `Kind`：`image`（圖片作品，預設值）或 `book`（學生上傳的故事本 PDF）。這兩種
  ...
  故事本的 `ImageURL` 一律留空、`NeedsManualPublish` 一律為 `FALSE`：PDF 沒辦法用
  `lh3.googleusercontent.com` 這種圖片網址顯示，所以下載固定走後端代理，也因此
  不需要 Drive 的公開分享權限、不會出現「審核中」狀態。
...
- **刪掉已停用功能留下的分頁**：投票與留言功能都已移除，`StoryRounds`、
  `StoryVotes`、`HonorBoard`、`StoryChain`、`Comments` 這五個分頁不再有任何程式碼
  會用到。要清掉請先
...
- [ ] 我的頁面：上傳故事本 PDF 成功後，會出現在「故事本」分頁，且狀態直接是
      「已上架」而不是「審核中」（故事本不走 Drive 公開分享，所以不需要審核）
- [ ] 故事接龍：按住每一格上方的「⠿ 拖曳排序」把手可以改變順序，放開後會自動儲存；
      用滑鼠和用 iPad 觸控都要能拖
- [ ] 我的頁面：圖片達 94 張或故事本達 8 本時，上方數量統計變紅字並出現提醒
- [ ] 我的頁面：圖片達 100 張或故事本達 10 本時，送出會被擋下並跳提示
- [ ] 畫廊：可切換「圖片 / 故事本」兩個分頁，切到故事本時「AI 工具」下拉會消失
- [ ] 畫廊：公開的故事本會出現在故事本分頁，點進去可以下載 PDF
- [ ] 畫廊：滑鼠移到卡片上，縮圖會放大預覽（手機沒有這個效果，屬正常）
- [ ] 作品詳細視窗：按讚在名字右邊、沒有留言區，圖片較高時右欄會自己捲動而不是
      在下面留一大片空白
